...
狀態（2026-09-06，**底層已驗、agent 層卡憑證**）：

✅ **自癒所需的底層條件已實測成立**（這是「補得成」的前提，不是補生成本身）：
- 缺口日 `2026-09-05` 在保留期內，`harvest.py` 對它回 **21 筆骨架**＝有料可補。
- 保留期外的 `2026-01-01` 回 **0 筆、exit 0**＝空不是錯，對應「標無記錄而非報錯」那條分支的底層行為。

❌ **agent 層未驗**，且**刻意沒有用兩種捷徑**：
- 🚫 不照規格字面「刪掉某天的日報」——維護者本機**根本沒有 `~/.claude/journal/`**（這三支從未實際使用），
  沒有東西可刪；若對真實 journal 動手，那是刪使用者資料，不在測試該做的事情裡。
- 🚫 不把 `~/.claude/.credentials.json` 複製進暫存目錄去餵隔離環境的 agent。

實際做到哪：隔離 `HOME` ＋ 隔離專案建好、14 支裝妥、放了 6 份合成日報並刻意缺 `2026-09-05`、
以 **junction**（不複製、事後移除並確認來源 28 個專案完好）讓隔離環境讀得到真實 transcript。
最後一步 `claude -p` 回 **`Not logged in · Please run /login`** ——
與 CROSS-05 的 gemini 那格**同一類阻礙**：隔離環境沒有 agent 憑證。
解除路徑二選一，都是使用者的決定：① 提供可用於隔離環境的 API key；
② 授權直接對真實 `~/.claude/journal/` 跑一次（會產生 7 份日報＋1 份週報，並消耗額度）。
