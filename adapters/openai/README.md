...
| recap | ✅ | ⚠️ | 純規則,四欄回報格式原封搬。差別在**刷新那步**:Codex CLI 有 shell 與版本控制,六項刷新清單全跑得動;ChatGPT 網頁版沒有本機環境,只剩回報格式,刷新不了的項目一律標「未驗證」——這正是 skill 明文要求的行為,不是降級失敗。<br>Pure rules; the four-column report ports verbatim. The difference is the *refresh* step: Codex CLI has a shell and version control, so the whole checklist runs; ChatGPT web has no local environment, leaving only the report format — anything it cannot refresh is labelled "unverified", which is exactly what the skill mandates, not a degraded failure. |
...
for s in save-all dropoff pickup recap damage-report blind-review ai-review token-optimizer; do
  echo -e "\n\n<!-- MyR2D2: $s -->" >> AGENTS.md
