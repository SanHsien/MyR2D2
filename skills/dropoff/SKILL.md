...
4. **即時門鈴**（選用 — 環境有跨 session 傳訊能力才跑；Claude Code v2.1.224+ 的 cross-session messaging，其他工具自動跳過）：
   ...
   - ② 否則列出可達的 session（`ListAgents`／`/list-agents`；桌面版用 session 管理工具的 list），找工作目錄或名稱對得上目標專案、且**最近活躍**（建議 7 天內）的那個。候選全都過期、或同專案多個相符 → **列出來問使用者，不要自動挑**——喚醒沉睡的 session 會重載大量舊 context，而且舊脈絡可能立刻開始行動。
