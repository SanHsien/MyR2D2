...
特殊問題字元）。Linux 由 CI（ubuntu-latest）跑同一份矩陣；Windows 11 的 Git Bash 會跑完整
行為案例，但 NTFS 無法提供 POSIX `0600` mode-bit 證據，因此該一項明確略過並由 Linux CI 權威驗證。
**免費方案帳號未實測**；**真實後端的 ok 路徑僅單次實測（Codex CLI），尚未逐項回歸** ——
沒驗過的一律別當保證。
