...
⁷ `new-mission` and `ai-search` were adopted in v0.8.0 from upstream `tingyulu/MyR2D2` v0.7.3; v0.8.1–v0.8.3 added the Windows fixes and the measurements. Almost every rating above is now **measured**: the install layer on `codex`, `claude-code` and `gemini-cli` at **14/14, 0 Skipped** each (run by every gate); **Gemini's discovery layer**, 0 skills listed while untrusted and all **14 `[Enabled]`** once trusted (0.58.0); the **execution layer**, with `dropoff` run end-to-end on both Codex CLI and Claude Code CLI and the resulting handoff cards checked field-by-field against the spec (CROSS-05 — the first time this repo has ever measured it); `ai-search`'s 44-item behavior matrix on every push; and `ai-search`'s **`ok` path on both backends** (the default Codex built-in search, and a pluggable one whose answer named a newer Codex release than the one installed locally — proof it really searched the live web), with failure classification also exercised against a real backend. **Only two things remain unmeasured**: `gemini-cli`'s execution layer (needs Google credentials — the user's call), and the fact that the execution-layer spot check covers `dropoff` only, so it does not extend to skills needing an external connector or subagents. `ai-search` needs a backend that **actually searches the web** (Codex CLI's built-in search by default, swappable via `AI_SEARCH_CMD` — but the replacement must also search; a plain LLM just fills from stale knowledge). No backend or not signed in → `skipped_*` and exit 0, so automation that only checks exit codes reads "skipped" as success; parse the final `AI_SEARCH_STATUS:` line on stdout to tell them apart. Per-item evidence: [docs/TEST_PLAN.md](docs/TEST_PLAN.md) section F, CROSS-02 and CROSS-05.
