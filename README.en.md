...
⁷ `new-mission` and `ai-search` were adopted in v0.8.0 from upstream `tingyulu/MyR2D2` v0.7.3; v0.8.1 added the Windows fixes and the measurements below. The ratings above come from **three different kinds of source and must not be conflated**: ① **measured by this fork** — the install layer on `codex`, `claude-code` and `gemini-cli`, **14/14 with 0 Skipped** on each (isolated Windows projects, `tools/windows_agent_smoke.ps1`, run by every gate); `ai-search`'s 44-item behavior matrix on every push in Windows Git Bash and Linux CI; and the real Codex backend's `failed_quota` path exercised on Windows (classified correctly). ② **not measured** — the real backend's `ok` path: this machine's account was over its usage limit at test time, so **this fork has never observed a successful search**; upstream has a single-run record of one. ③ **inferred** — every remaining cell (the Cowork / Gemini / Codex / ChatGPT **execution** layer) is the same kind of claim as ⁶, inferred from "rules-only, zero tool deps", not measured; Gemini's **discovery** layer is likewise unverified (the Gemini CLI is not installed here — the install layer is the `npx skills` installer's behavior and does not depend on that CLI being present). `ai-search` needs a backend that **actually searches the web** (Codex CLI's built-in `web_search` by default, swappable via `AI_SEARCH_CMD` — but the replacement must also search; a plain LLM just fills from stale knowledge). No backend or not signed in → `skipped_*` and exit 0, so automation that only checks exit codes reads "skipped" as success; parse the final `AI_SEARCH_STATUS:` line on stdout to tell them apart. Per-item evidence and provenance: [docs/TEST_PLAN.md](docs/TEST_PLAN.md) section F.
