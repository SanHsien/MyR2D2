...
⁷ `new-mission` and `ai-search` were adopted in v0.8.0 from upstream `tingyulu/MyR2D2` v0.7.3; v0.8.1–v0.8.2 added the Windows fixes and the measurements below. The ratings above come from **two different kinds of source and must not be conflated**: ① **measured by this fork** — the install layer on `codex`, `claude-code` and `gemini-cli`, **14/14 with 0 Skipped** on each (isolated Windows projects, run by every gate); **Gemini's discovery layer**, listing 0 skills while the folder is untrusted and **all 14 `[Enabled]`** once trusted (gemini-cli 0.58.0); `ai-search`'s 44-item behavior matrix on every push in Windows Git Bash and Linux CI; and `ai-search`'s **`ok` path through a pluggable backend** (the answer named a newer Codex release than the one installed locally — proof it really searched the live web), with the default Codex backend verified to classify failures correctly. ② **inferred, not yet measured** — each tool's **execution** layer (whether a trigger actually produces SKILL.md-conformant behavior) is the same kind of claim as ⁶, inferred from "rules-only, zero tool deps"; and the **`ok` path on the default codex `web_search` backend** is still unverified (the account was over its usage limit at test time). `ai-search` needs a backend that **actually searches the web** (Codex CLI's built-in `web_search` by default, swappable via `AI_SEARCH_CMD` — but the replacement must also search; a plain LLM just fills from stale knowledge). No backend or not signed in → `skipped_*` and exit 0, so automation that only checks exit codes reads "skipped" as success; parse the final `AI_SEARCH_STATUS:` line on stdout to tell them apart. Per-item evidence and provenance: [docs/TEST_PLAN.md](docs/TEST_PLAN.md) section F and CROSS-02.

- **Gemini CLI / Codex CLI**: install & discovery layers verified — Gemini measured on 0.58.0 in both states (0/14 while untrusted, all 14 `[Enabled]` once trusted); the **trusted-folder gate is silent, so if skills don't show up, trust the project folder first**. Execution layer untested for both.
